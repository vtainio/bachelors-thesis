...
The application uses Bluetooth beacons to identify the strangers and log the encounters to a web server. This enables users to interact with the strangers also outside of the sports activity itself. They don't have to approach the strangers while doing sports. They can later on interact with people they have met while doing sports and try to find like-minded people to do sports with. 
...
The questions weren't meant to be strict but to just serve as a guideline for the discussion. If new interesting topics emerged while interviewing, the idea was to go forward with them without thinking too much about the guideline.

\subsection{Results}

In total, three people were interviewed for this thesis. The interviews ranged from 20 to 30 minutes in length. A few demographic questions were included in the interviews. Two of the participants were female and one was  a male. One of the participants did sports four to five times a week, one two to three times a week and one did sports rarely (less than weekly). Their sports activities were gym, various group exercises (e.g. spinning), martial arts and jogging.
...
% TODO Remove external chat service from the graph.
...
The goal of this thesis was to research how humans interact with familiar strangers and also create a prototype application for sports, which will use these findings and enable people to find like-minded strangers to get to know them and to do sports with. Based on the literature review and the interviews conducted for this thesis, a prototype Android application has been created and it is functional. It is now possible to do various types of user studies with the open source application.

Based on multiple research from , people are generally interested in getting to know strangers using these kinds of social networking applications. By using this prototype application, people are able to get new friends to do sports with and therefore benefit from the research benefits of doing sports with another person.

The interviews that were conducted for this thesis gave various good insights into the design of the prototype application. Although the amount of interviews was low, the results were valuable and useful both in the design and the implementation phase of the prototype application.

The prototype application is easy to take into use. The backend service is also very easy to replace to the application so that it's easier to monitor the generated data. You can change the backend by merely changing a single line in the source code. The prototype was also open sourced with the idea that someone else can take the project forward and do user studies that are lacking in this thesis.
